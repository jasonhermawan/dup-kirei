...
const ServiceCard = ({image, title, price, unit, children} : ServiceProps) => {
  return (
    <Box className="service-card">
      ...
      <Text size="36px" fw="600" mb="20px" visibleFrom="lg">IDR {price?.toLocaleString()},-<span style={{fontSize: "14px", color: 'rgba(102, 112, 133, 1)', fontWeight: "400"}}> / {unit}</span></Text>
      <Text size="16px" fw="500" mb="20px" hiddenFrom="lg">{title}</Text>
      <Text size="24px" fw="600" mb="20px" hiddenFrom="lg">IDR {price?.toLocaleString()},-<span style={{fontSize: "14px", color: 'rgba(102, 112, 133, 1)', fontWeight: "400"}}> / {unit}</span></Text>
